...
object StringUtils extends Logging {
  ...
  /**
   * Removes comments from a SQL command.
   ...
   */
  private def stripComment(command: String, replaceWithWhitespace: Boolean = false): String = {
  ...
  /**
   * Part of a multi-statement sql query.
   *
   * @param query The sub-query string.
   * @param beginIndex Where this sub-query begins in the larger multi-statement query.
   */
  case class SubQuery(query: String, beginIndex: Int)
   ...
   *
   * @param cleanText The query text, already stripped of comments.
   */
  private def isCreateSqlStoredProcedureText(cleanText: String): Boolean = {
    cleanText.startsWith("CREATE") && cleanText.contains("PROCEDURE") &&
      cleanText.contains("BEGIN") &&
      (cleanText.endsWith("END") || cleanText.endsWith("END;"))
  }
  ...
  def splitSemiColonWithIndex(line: String, enableSqlScripting: Boolean): List[SubQuery] = {
    ...
    val ret = new ArrayBuffer[SubQuery]()
      ...
      if (line.charAt(index) == '\'' && !insideComment) {
        ...
        if (insideSingleQuote || insideDoubleQuote || insideBacktick || insideComment) {
          ...
          if (isStatement) {
            ...
            ret.append(SubQuery(line.substring(beginIndex, index), beginIndex))
          }
          beginIndex = index + 1
          isStatement = false
        }
    ...
    if (!endOfBracketedComment && (isStatement || insideBracketedComment)) {
      ret.append(SubQuery(line.substring(beginIndex), beginIndex))
    }
    ret.toList
  }
}
