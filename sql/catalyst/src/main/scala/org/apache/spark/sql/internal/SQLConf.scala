  ...
  val PIPELINES_WATCHDOG_MIN_RETRY_TIME_IN_SECONDS =
    buildConf("spark.sql.pipelines.execution.watchdog.minRetryTime")
      .doc(
        ...
        "restart the flow. The interval between flow restarts doubles with every stream failure " +
        "up to the maximum value set in `pipelines.execution.watchdog.maxRetryTime`."
      )
      .version("4.1.0")
      .timeConf(TimeUnit.SECONDS)
      .createWithDefault(5)
