...
abstract class GraphExecution(
  ...
  def startFlow(flow: ResolvedFlow): Option[FlowExecution] = {
    try {
      val physicalFlow =
        flowPlanner.plan(flow = graphForExecution.resolvedFlow(flow.identifier))
  ...
  /**
   * Starts batch and stream listeners to listen to events and metrics.
   */
  def start(): Unit = {
