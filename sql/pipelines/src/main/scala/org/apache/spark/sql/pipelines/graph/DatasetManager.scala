 ...
 *
 * TODO: Add logic to materialize persisted views.
 */
object DatasetManager extends Logging {
  ...
  def materializeDatasets(
    ...
    val (allRefreshTables, refreshTableIdentsSet, fullRefreshTableIdentsSet) =
   ...
   */
  protected def materializeTable(
      virtualizedConnectedGraphWithTables: DataflowGraph,
      table: Table,
  ...
  case class TableRefreshType(table: Table, isFullRefresh: Boolean)
